...
load_data <- function(data_dir, time_string, PATIENT_DATE, DATA_CREATION_DATE){
    data_file = file.path(data_dir, PATIENT_DATE, sprintf('influenza_patient_data_%s_%s.csv', time_string, DATA_CREATION_DATE))
    print(data_file)
...
enrichment_test <- function(phecode_list, known_risk_factors, df, 
    ...
    for (i in seq(total)){
        ...
        # frequency in covid
        n_COVID = sum(df['INFLUENZA'])
        n_COVID_phecode = sum(df['INFLUENZA']*df[target_phecode])
        ...
        result_row= list(membership_string, outcome_string, cohort_string, adjust_string, 
                      ...
                      freq_SEVERE_phecode, freq_INPATIENT_phecode, freq_COVID_phecode, 
...
compare_list = list(c('SEVERE', 'INPATIENT'),c('INPATIENT', 'INFLUENZA'))
sheet_list = list()
...
for (compare in compare_list){
    ...
    # save results
    file_name = sprintf('influenza_phecode_firth_logistic_%sin%s_%s_M1M2_%s_%s.xlsx', 
