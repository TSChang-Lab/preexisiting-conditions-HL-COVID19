...
enrichment_test <- function(phecode_list, known_risk_factors, df, 
                            ...
                            membership_string,adjust_string = 'adj_SA'){
    # subset data frame 
    ...
    for (i in seq(total)){
        ...
        if (sum(analysis_df[,target_phecode] * analysis_df[,'HispanicORLatino']) ==0) next

        model_covariates = c(covariates,paste('HispanicORLatino', target_phecode, sep = '*'))
        ...
        interact_term = paste('HispanicORLatino', target_phecode, sep = ':')
        p = model$prob[interact_term]
        oddsratio = round(exp(coef(model)[interact_term]),2)
        ci_lower = round(exp(confint(model)[interact_term,1]),2)
        ci_upper = round(exp(confint(model)[interact_term,2]),2)
...
compare_list = list(c('SEVERE', 'INPATIENT'),
                    c('INPATIENT', 'COVID'),
                    c('COVID', 'TESTED'))
...
for (compare in compare_list){
    ...
    for(race_ethnicity in race_ethnicity_group){
        file_name = sprintf('covid_phecode_firth_logistic_%sin%s_%s_M1M2_%s_%s.xlsx', 
                            outcome_string, cohort_string, time_string, '09012020', race_ethnicity)
        in_file = file.path(result_dir, PATIENT_DATE, file_name)
        uni_results = read.xlsx(in_file, sheet = 'M1')

        uni_list = c(uni_results[uni_results[,'Bonferr.Sig.'] == '1','Phecode'])
        phecode_list = c(phecode_list, uni_list)

    }
    phecode_list = unique(phecode_list)
    if(identical(phecode_list, character(0) )){
        print(phecode_list)
        next
    }else{
        M1_phecodes = M2_phecodes = phecode_list
    }
    
    file_name = sprintf('covid_phecode_firth_logistic_%sin%s_%s_M1M2_%s_HispanicInteraction.xlsx', 
                        outcome_string, cohort_string, time_string, ANALYSIS_DATE)
    ...
    M2_results = enrichment_test(M2_phecodes, known_risk_factors, df, 
                                outcome_string, cohort_string, time_string, 
                                membership_string,adjust_string = 'adj_SA_krf')
